...
def check_success_file_price_log (context):
    insert_dq_checks_results = PostgresOperator(
        task_id="success_file_price_log",
        sql="""
            INSERT INTO dq_checks_results
            values ('price_log', 'file_sensor' ,current_date, 0 )
          """
    )

def check_failure_file_price_log (context):
    insert_dq_checks_results = PostgresOperator(
        task_id="failure_file_user_price_log",
        sql="""
            INSERT INTO dq_checks_results
            values ('price_log', 'file_sensor' ,current_date, 1 )
          """
    )
...
with DAG(dag_id="Sprin4_Task71", schedule_interval="@daily", default_args=default_args, catchup=False) as dag:
    ...
    with TaskGroup(group_id='group1') as fg1:
        ...
        f4 = FileSensor(task_id='waiting_for_file_price_log', fs_conn_id ='fs_local', filepath=str(datetime.now().date()) + "price_log.csv", poke_interval=5 , on_success_callback = check_success_file_price_log, on_failure_callback =  check_failure_file_price_log)
    with TaskGroup(group_id='group2') as fg2:
        load_customer_research = PythonOperator(task_id='load_customer_research', python_callable=load_file_to_pg, op_kwargs={'filename': str(datetime.now().date()) + "customer_research.csv",'pg_table': 'staging.customer_research'})
        load_user_order_log = PythonOperator(task_id='load_user_order_log', python_callable=load_file_to_pg, op_kwargs={'filename': str(datetime.now().date()) + "user_order_log.csv",'pg_table': 'staging.user_order_log'})
        load_user_activity_log = PythonOperator(task_id='load_user_activity_log', python_callable=load_file_to_pg, op_kwargs={'filename': str(datetime.now().date()) + "user_activity_log.csv",'pg_table': 'staging.user_activity_log'})
        load_price_log = PythonOperator(task_id='load_price_log', python_callable=load_file_to_pg, op_kwargs={'filename': str(datetime.now().date()) + "price_log.csv",'pg_table': 'staging.price_log'})
        ...
        sql_check4 = SQLValueCheckOperator(task_id="check_row_count_user_activity_log", sql="Select count(distinct(customer_id)) from user_activity_log" , pass_value=3,on_success_callback = check_success_insert_user_activity_log, on_failure_callback =  check_failure_insert_user_activity_log )    
